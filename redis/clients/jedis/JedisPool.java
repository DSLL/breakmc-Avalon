...
public class JedisPool extends Pool<Jedis>
{
    ...
    public JedisPool(final String host) {
        ...
        if (uri.getScheme() != null && uri.getScheme().equals("redis")) {
            ...
            this.internalPool = (GenericObjectPool<T>)new GenericObjectPool((PooledObjectFactory)new JedisFactory(h, port, 2000, password, database, null), new GenericObjectPoolConfig());
        }
        else {
            this.internalPool = (GenericObjectPool<T>)new GenericObjectPool((PooledObjectFactory)new JedisFactory(host, 6379, 2000, null, 0, null), new GenericObjectPoolConfig());
        }
    }
    ...
    public JedisPool(final GenericObjectPoolConfig poolConfig, final String host, final int port, final int timeout, final String password, final int database, final String clientName) {
        super(poolConfig, (PooledObjectFactory)new JedisFactory(host, port, timeout, password, database, clientName));
    }
    ...
    public JedisPool(final GenericObjectPoolConfig poolConfig, final URI uri, final int timeout) {
        super(poolConfig, (PooledObjectFactory)new JedisFactory(uri.getHost(), uri.getPort(), timeout, JedisURIHelper.getPassword(uri), (JedisURIHelper.getDBIndex(uri) != null) ? JedisURIHelper.getDBIndex(uri) : 0, null));
    }
